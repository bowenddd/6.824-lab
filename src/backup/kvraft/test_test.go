...
func GenericTest(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	for i := 0; i < 3; i++ {
		...
		if maxraftstate > 0 {
			...
			sz := cfg.LogSize()
			if sz > 1000*maxraftstate {
				t.Fatalf("logs were not trimmed (%v > 1000*%v)", sz, maxraftstate)
			}
		}
		if maxraftstate < 0 {
...
func TestSnapshotRPC3B(t *testing.T) {
	...
	sz := cfg.LogSize()
	if sz > 10*maxraftstate {
		t.Fatalf("logs were not trimmed (%v > 10*%v)", sz, maxraftstate)
	}
...
func TestSnapshotSize3B(t *testing.T) {
	...
	sz := cfg.LogSize()
	if sz > 100*maxraftstate {
		t.Fatalf("logs were not trimmed (%v > 100*%v)", sz, maxraftstate)
	}
